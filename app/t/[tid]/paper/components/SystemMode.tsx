...
export function SystemMode({ tournamentId, participants, initRounds, initQuestions, initSubs }: Props) {
  ...
  const saveAllSubs = async () => {
    ...
    for (const p of participants) {
      ...
      await supabase.from('paper_submissions').upsert({
        paper_round_id: selectedRoundId, participant_id: p.id,
        raw_score: sub.raw_score, proximity_error: sub.proximity_error,
        chain1: sub.chain1, chain2: sub.chain2, chain3: sub.chain3,
  ...
  const createRound = async () => {
    ...
    const { data } = await supabase.from('paper_rounds').insert({ tournament_id: tournamentId, name: newRoundName.trim(), order_index: rounds.length }).select().single()
  ...
  const addQuestion = async () => {
    ...
    const { data } = await supabase.from('paper_questions').insert({ paper_round_id: selectedRoundId, order_index: roundQuestions.length + 1, question_text: newQ.text.trim(), correct_answer: newQ.answer.trim(), question_type: newQ.type }).select().single()
    if (data) { setQuestions(prev => [...prev, data]); setNewQ({ text: '', answer: '', type: 'regular' }) }
  }
          ...
          {rounds.map(r => (
            <div key={r.id} className="flex items-center gap-1">
              <button onClick={() => setSelectedRoundId(r.id)}
                className={cn('px-3 py-1.5 rounded-lg text-sm font-medium transition-colors', selectedRoundId === r.id ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-300 hover:text-white')}>
                {r.name}
              </button>
              <button onClick={() => deleteRound(r.id)} className="text-zinc-700 hover:text-red-400"><Trash2 size={13} /></button>
            </div>
          ))}
        </div>
        <div className="flex gap-2">
          <input value={newRoundName} onChange={e => setNewRoundName(e.target.value)} onKeyDown={e => e.key === 'Enter' && createRound()} placeholder="新しいラウンド名" className="flex-1 bg-zinc-800 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500" />
          <button onClick={createRound} disabled={!newRoundName.trim()} className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-lg disabled:opacity-40">
            <Plus size={14} /> 追加
          </button>
        </div>
      </div>
        ...
        <>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-3">
              ...
              : <div className="space-y-1">
                  {roundQuestions.map((q, i) => (
                    <div key={q.id} className="flex items-start gap-2 bg-zinc-800 rounded-lg px-3 py-2">
                      ...
                      <button onClick={() => deleteQuestion(q.id)} className="text-zinc-600 hover:text-red-400 shrink-0"><Trash2 size={13} /></button>
                    </div>
                  ))}
                </div>
            }
            <div className="space-y-2 pt-1 border-t border-zinc-800">
              <div className="flex gap-2">
                <input value={newQ.text} onChange={e => setNewQ(p => ({ ...p, text: e.target.value }))} placeholder="問題文" className="flex-1 bg-zinc-800 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500" />
                <select value={newQ.type} onChange={e => setNewQ(p => ({ ...p, type: e.target.value }))} className="bg-zinc-800 text-white text-sm rounded-lg px-2 py-2 focus:outline-none">
                  <option value="regular">通常</option>
                  <option value="proximity">近似値</option>
                </select>
              </div>
              <div className="flex gap-2">
                <input value={newQ.answer} onChange={e => setNewQ(p => ({ ...p, answer: e.target.value }))} placeholder="正答（任意）" className="flex-1 bg-zinc-800 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500" />
                <button onClick={addQuestion} disabled={!newQ.text.trim()} className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-lg disabled:opacity-40">
                  <Plus size={14} /> 追加
                </button>
              </div>
            </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-800">
              ...
              <button onClick={saveAllSubs} disabled={saving} className="flex items-center gap-2 px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-lg font-bold disabled:opacity-50">
                      ...
                      <tr key={p.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                        ...
                        {(['raw_score', 'proximity_error', 'chain1', 'chain2', 'chain3'] as const).map(field => (
                          <td key={field} className="px-2 py-1.5 text-center">
                            <input type="number" value={sub[field] ?? ''} onChange={e => updateSubLocal(p.id, field, e.target.value === '' ? null : Number(e.target.value))}
                              className="w-full bg-zinc-800 text-white text-center text-sm rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500 [appearance:textfield]" placeholder="—" />
                          </td>
                        ))}
                      </tr>
                    )
